...
const spacesEnd = (
	...
	let cursor = start;
	while (cursor < end && cursor - start < maximum && value[cursor] === " ")
		cursor += 1;
	return cursor;
};
...
const listMarkerEnd = (
	...
	if (marker === "-" || marker === "+" || marker === "*") {
		...
		while (cursor < end && cursor - digitStart < 9 && /\d/.test(value[cursor]))
			cursor += 1;
		if (
			cursor === digitStart ||
	...
	while (cursor < end && (value[cursor] === " " || value[cursor] === "\t"))
		cursor += 1;
...
const markdownFenceAt = (
	...
	if (marker === "`" && value.slice(runEnd, contentEnd).includes("`"))
		return null;
...
export const normalizeTexMathDelimiters = (value: string): string => {
	...
	while (cursor < value.length) {
		if (cursor === lineStart) {
			const codeLineEnd =
				opening == null ? indentedCodeLineEnd(value, lineStart) : null;
			if (codeLineEnd != null) {
				cursor = codeLineEnd;
				lineStart = cursor;
				continue;
		...
		const kind =
			delimiterMarker === "(" || delimiterMarker === ")" ? "inline" : "display";
...
export const remarkSingleDollarTextMath = () => {
	const walk = (node: MdastNode) => {
		if (
			...
			SKIP_CHILDREN_NODE_TYPES.has(node.type)
		) {
			return;
		}
