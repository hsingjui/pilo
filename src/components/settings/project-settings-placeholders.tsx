...
export function WorkspaceSettings() {
	return (
		<div className="space-y-3">
			<SettingsSection
				title="工作区"
				description="Workspace 表示开发目录，并绑定一个 Connection。"
			>
				<SettingsRow
					label="添加工作区"
					helper="支持 Local Folder、WSL 路径和 SSH 远程目录。"
				>
					<SettingsStatus muted>规划中</SettingsStatus>
				</SettingsRow>
				<SettingsRow
					label="发现已有 Pi 工作区"
					helper="扫描现有 Pi 使用目录并允许选择导入，不复制远程项目到本地。"
				>
					<SettingsStatus muted>规划中</SettingsStatus>
				</SettingsRow>
				<SettingsRow
					label="最近使用"
					helper="维护 Recent Workspaces，并缓存 cwd、Git branch、Pi version 等环境信息。"
				>
					<SettingsStatus muted>规划中</SettingsStatus>
				</SettingsRow>
			</SettingsSection>
