...
pub fn ssh_connection_save(
    ...
    if password_auth {
        if let Some(password) = request.password.as_deref() {
            super::credentials::set_ssh_password(&request.connection.id, password)?;
        } else if !super::credentials::has_ssh_password(&request.connection.id) {
            return Err("SSH password is required".to_owned());
        }
    } else {
        super::credentials::delete_ssh_password(&request.connection.id)?;
    }
...
pub fn ssh_connection_remove(app: AppHandle, id: String) -> Result<(), String> {
    ...
    super::credentials::delete_ssh_password(&id)?;
    Ok(())
}
