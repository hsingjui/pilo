...
# Sub-agents are disabled in this project: skip curation and use `start --allow-empty-context`.
...
```
Phase 1: Plan    → create a task only when the user asks, then write planning artifacts
...
- Do NOT ask whether to create a task. Create one only when the user explicitly asks for a Trellis task.
- No task: handle the request inline this turn; do not run Trellis phases.
- Task created: enter planning. Task creation is not implementation approval; planning still happens first.
...
- `implement.jsonl` / `check.jsonl` — spec and research manifests for sub-agent context (unused in inline mode; sub-agents are disabled). They do not replace `implement.md`.
...
No active task. Do NOT ask whether to create a Trellis task.
Create a Trellis task only when the user explicitly asks for one; otherwise just do the work inline this turn.
If the user does ask: create the task, then enter planning.
[/workflow-state:no_task]
...
### Phase 1: Plan
- 1.0 Create task `[required · once]` (only when the user explicitly asks to create a task)
- 1.1 Requirement exploration `[required · repeatable]` (`prd.md`; complex tasks also need `design.md` + `implement.md`)
- 1.2 Research `[optional · repeatable]` (in the main session; write findings to `research/`)
- 1.3 Configure context `[required · once]` — skipped in inline mode; context is loaded by `trellis-before-dev` in Phase 2
...
Sub-agents are disabled: do all work in the main session. Ignore any sub-agent dispatch instructions that remain elsewhere in this file.

[workflow-state:in_progress]
No sub-agents: implement and check in the main session.
...
- Sub-agents are disabled: do all implementation and checking in the main session.
...
Goal: create a task only when the user asks, and produce the planning artifacts required before implementation.

#### 1.0 Create task `[required · once]`

Create the task directory only when the user explicitly asks for a Trellis task. The command sets status to `planning`, writes `task.json`, creates a default `prd.md`, and auto-targets the new task when session identity is available:
...
Do the research in the main session directly and write findings into `{TASK_DIR}/research/`. Sub-agents are disabled.
...
Sub-agents are disabled, so skip context curation. Context is loaded directly by the `trellis-before-dev` skill in Phase 2.
...
Sub-agents are disabled, so the `implement.jsonl` / `check.jsonl` context gate does not apply — pass `--allow-empty-context` to bypass it.
