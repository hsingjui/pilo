...
- 日志路径使用 Tauri 的 `app_log_dir()`，About 页面通过前端 `appLogDir()` 获取同一路径。
...
- R2 日志落盘到 `app_log_dir()`，单文件上限 5 MiB，使用 `RotationStrategy::KeepSome(4)` 做有界轮转。
...
- R5 前端在 `src/main.tsx` 通过单一 `attachLogger()` 监听器回放后端日志，并将 webview 的 `console.error/warn` 转发进同一日志文件。
- R6 About 设置页增加“打开日志目录”入口，复用已注册的 `opener` 插件。
- R7 `src-tauri/capabilities/default.json` 增加 `log:default` 与 `opener:allow-open-path` 权限。
...
- [ ] AC1 release 构建运行后，`app_log_dir()` 下存在日志文件，且包含启动/后台任务失败记录。
...
- [ ] AC6 About → 打开日志目录可正确打开 `app_log_dir()`。
...
- 实现时需确认：日志回放不得调用被包装的 console 方法形成回显循环，且 webview 高频 `console.log` 不得进入文件；`KeepSome` 的保留语义以实际版本为准。
